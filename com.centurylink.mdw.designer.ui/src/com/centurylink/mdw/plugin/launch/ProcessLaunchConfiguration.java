...
public class ProcessLaunchConfiguration extends WorkflowLaunchConfiguration {
    ...
    public static final String PROCESS_LAUNCH = "Process Launch";
    ...
    public void launch(ILaunchConfiguration launchConfig, String mode, ILaunch launch,
        ...
        if (workflowProject == null) {
            showError("Can't locate workflow project: '" + wfProjectName + "'.", PROCESS_LAUNCH,
                    null);
            return;
        }

        if (mode.equals(ILaunchManager.DEBUG_MODE)
                && !connectForDebug(workflowProject, launchConfig)) {
            return;
        }
        ...
        if (process == null) {
            ...
            if (procVO == null) {
                showError("Can't locate process '" + processName + " v" + processVersion + "' in "
                        + wfProjectName + ".", PROCESS_LAUNCH, workflowProject);
                return;
            }
            else {
        ...
        launchProcess(process, masterRequestId, owner, new Long(ownerId), synchronous,
                responseVarName, variableValues, null, showLogs, liveView);
    }
    ...
    protected void launchProcess(WorkflowProcess process, String masterRequestId, String owner,
            ...
            Map<String, String> parameters, Long activityId, boolean showLogs, boolean liveView) {
    ...
    private ILaunchConfiguration findExistingDebugLaunchConfig(
        ...
        for (ILaunchConfiguration launchConfig : configs) {
            ...
            for (Map.Entry<?, ?> connectAttr : connectAttrMap.entrySet()) {
                if (!"timeout".equals(connectAttr.getKey())
                        && !connectAttr.getValue().equals(wcConnectAttrMap.get(connectAttr.getKey()))) {
                    mapMatches = false;
                    continue;
                }
            }
