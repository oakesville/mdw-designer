...
public class ProcessInstanceListView extends ViewPart {
    ...
    public void createPartControl(Composite parent) {
        ...
        ProcessInstanceLabelProvider labelProvider = new ProcessInstanceLabelProvider();
        tableViewer.setLabelProvider(labelProvider);

        ProcessInstanceActionGroup actionGroup = new ProcessInstanceActionGroup(this);
    ...
    private void refreshProjects() {
        ...
        if (workflowProjects != null && !workflowProjects.isEmpty()) {
            for (WorkflowProject wfProject : workflowProjects)
                projectCombo.add(wfProject.getName());
        ...
        BusyIndicator.showWhile(getSite().getShell().getDisplay(), new Runnable() {
            public void run() {
                ...
                if (editor instanceof ProcessEditor) {
                    ...
                    CanvasCommon localCanvas = processEditor.getProcessCanvasWrapper().getCanvas();
                    PanelBusyIndicator pbi = new PanelBusyIndicator(
                            getSite().getShell().getDisplay(), localCanvas);
                    try {
