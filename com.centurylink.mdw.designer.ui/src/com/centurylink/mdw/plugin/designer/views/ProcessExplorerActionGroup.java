...
public class ProcessExplorerActionGroup extends ActionGroup {
    private static final String ICON_DOC_GIF = "icons/doc.gif";
    private static final String OTHER = "Other";
    private static final String OTHER_DOT = "Other...";
    private static final String OPEN_PROCESS_INSTANCE = "Open Process Instance";
    private static final String CAMEL_GIF = "camel.gif";
    private static final String RUN_ACTION = "Run Action";
    private static final String ICON_RUN_GIF = "icons/run.gif";
    private static final String PROCESS_DOT = "Process...";
    private static final String IMPORT_EXPORT = "ImportExport";
    private static final String ICON_PACKAGE_GIF = "icons/package.gif";
    private static final String MDW_ACTIONS = "MdwActions";
    private static final String ICONS = "icons/";
    private static final String ICON_PROCESS_GIF = "icons/process.gif";
    private static final String NOT_AUTHORIZED = "You are not authorized to import into project: ";
    ...
    private IAction newKotlinAction;
    ...
    public ProcessExplorerActionGroup(ProcessExplorerView view) {
        ...
        newCamelRouteAction = createNewAction(CamelRoute.class, "Camel Route", CAMEL_GIF);
        ...
        newKotlinAction = createNewAction(Kotlin.class, "Kotlin Source", "kotlin.gif");
    ...
    public void fillContextMenu(IMenuManager menu) {
        ...
        menu.add(new Separator());
        menu.add(new Separator(IMPORT_EXPORT));
        menu.appendToGroup(IMPORT_EXPORT, importMenu);
        importActionGroup.fillContextMenu(menu);

        if (!elements.isEmpty()) {
            if (exportApplies(selection)) {
                menu.appendToGroup(IMPORT_EXPORT, exportMenu);
            ...
            menu.add(new Separator(MDW_ACTIONS));

            if (runMenuApplies(selection)) {
                menu.appendToGroup(MDW_ACTIONS, runMenu);
            ...
            public void fillContextMenu(IMenuManager menu) {
                ...
                if (importWorkflowAssetApplies(selection)) {
                    if (element instanceof WorkflowAsset) {
                        ...
                        importWorkflowAssetAction
                                .setImageDescriptor(MdwPlugin.getImageDescriptor(ICON_DOC_GIF));
                    }
    ...
    private ActionGroup createRunActionGroup() {
        runMenu = new MenuManager("Run", MdwPlugin.getImageDescriptor(ICON_RUN_GIF),
    ...
    private ActionGroup createWebActionGroup() {
        ...
        return new ActionGroup() {
            ...
            public void fillContextMenu(IMenuManager menu) {
                ...
                IStructuredSelection selection = getSelection();
                if (webLaunchApplies(selection)
                        && selection.getFirstElement() instanceof WorkflowElement) {
    ...
    private IAction createTagVersionAction() {
        ...
        ImageDescriptor propsImageDesc = MdwPlugin.getImageDescriptor(ICON_PACKAGE_GIF);
        action.setImageDescriptor(propsImageDesc);
    ...
    private IAction createImportWorkflowAssetAction() {
        ...
        ImageDescriptor imageDesc = MdwPlugin.getImageDescriptor(ICON_DOC_GIF);
        action.setImageDescriptor(imageDesc);
            ...
            public void run() {
                ...
                try {
                    ...
                    if (!isAuthorized(element)) {
                        MessageDialog.openError(getViewSite().getShell(), RUN_ACTION,
                                "You are not authorized to run " + element.getName());
                        return;
                    }
                    view.setFocus();
                    ...
                    MessageDialog.openError(MdwPlugin.getShell(), RUN_ACTION,
                            "Server appears to be offline: " + restServer.getServerUrl());
                }
                catch (Exception ex) {
                    PluginMessages.log(ex);
    ...
    public boolean showPropertiesApplies(IStructuredSelection selection) {
        ...
        return element instanceof WorkflowProject || element instanceof WorkflowPackage
    ...
    public boolean incrementVersionApplies(IStructuredSelection selection) {
        ...
        return !(packageVersion.isDefaultPackage() || packageVersion.isArchived()
        ...
        if (selection.getFirstElement() instanceof WorkflowElement) {
            ...
            if (destPackage != null) {
                ...
                if (clipboardContents instanceof String) {
                    ...
                    if (clipString.startsWith("Process~")
    ...
    public boolean importProjectApplies() {
        return true;
    }
    ...
    public boolean runTestsApplies(IStructuredSelection selection) {
        if (selection.size() == 1 && selection.getFirstElement() instanceof WorkflowElement) {
            WorkflowElement element = (WorkflowElement) selection.getFirstElement();
            if (element instanceof WorkflowProject) {
                if (!((WorkflowProject) element).isRemote())
                    return true;
            ...
            if (element.getProject().isFilePersist() && element instanceof WorkflowPackage) {
                if (!element.getProject().isRemote())
                    return true;
    ...
    public boolean runApplies(IStructuredSelection selection) {
        if (selection.size() > 1) {
            ...
            WorkflowProject workflowProject = ((AutomatedTestCase) selection.getFirstElement())
                    .getProject();
    ...
    private void refreshRolePermissions(WorkflowProject project)
        ...
        if (!dao.isRolesRefreshed() && isVCS) {
            DesignerDataAccess localDao = new DesignerDataAccess(restServer, null, user, null,
                    project.isOldNamespaces(), isVCS);
    ...
    private boolean isAuthorized(WorkflowElement element) {
        if (element instanceof WorkflowProject)
            return ((WorkflowProject) element)
                    .isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION);
        if (element.getProject().isFilePersist() && element instanceof WorkflowPackage)
            return ((WorkflowPackage) element).isUserAuthorized(UserRoleVO.PROCESS_EXECUTION);
        if (element instanceof AutomatedTestSuite || element instanceof AutomatedTestCase) {
            if (!element.getProject().isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION))
                return false;
            else {
                WorkflowPackage pkg = element.getPackage();
                if (pkg != null && pkg.isUserAuthorized(UserRoleVO.PROCESS_EXECUTION))
        ...
        if (element instanceof WorkflowProcess || element instanceof ExternalEvent)
            return element.isUserAuthorized(UserRoleVO.PROCESS_EXECUTION);
        return false;
    }
}
