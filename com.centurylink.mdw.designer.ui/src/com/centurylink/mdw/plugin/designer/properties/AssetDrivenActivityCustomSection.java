...
public class AssetDrivenActivityCustomSection extends DesignSection implements IFilter {
    ...
    public boolean select(Object toTest) {
        ...
        if (testActivity.isManualTask()) // manual tasks have asset-driven widgets
                                     // but no custom section
            return false;

        if (testActivity.isSubProcessInvoke()) // so do subprocess launch activities
            return false;
    ...
    public void setAttribute(String name, String value) {
        ...
        StringBuilder customAttrs = new StringBuilder();
        int i = 0;
        for (Map.Entry<String, String> entry : valueMap.entrySet()) {
            customAttrs.append(entry.getKey()).append("=");
            customAttrs.append(entry.getValue());
