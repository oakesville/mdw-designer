...
public class PropertyEditor {
    ...
    private static final String UNITS_ATTR = "UNITS_ATTR";
    ...
    public void updateWidget(String value) {
        ...
        if (type.equals(TYPE_TEXT))
            ...
            ((Spinner) widget).setSelection(
                     newValue.length() == 0 ? 0 : Integer.parseInt(newValue));
    ...
    public PropertyEditor(WorkflowElement workflowElement, MbengNode mbengNode) {
        ...
        String lDefaultValue = mbengNode.getAttribute("DEFAULT");
        if (lDefaultValue != null && workflowElement instanceof Activity) {
            Activity activity = (Activity) workflowElement;
            if (type.equals(TYPE_TIMER)) {
                this.units = Units.valueOf(lDefaultValue);
                if (activity.getAttribute(mbengNode.getAttribute(UNITS_ATTR)) == null)
                    activity.setAttribute(mbengNode.getAttribute(UNITS_ATTR), lDefaultValue);
            }
            else {
                this.value = lDefaultValue;
        ...
        if (getType().equals(TYPE_COMBO) || getType().equals(TYPE_PICKLIST)) {
            if (source == null) {
                ...
                for (MbengNode optionNode = mbengNode
                    ...
                    if (optionNode.getAttribute("PARAMETER") != null
                        ...
                        if (parameterizedValueConverter.getOptions().size() < valueOptions.size()) {
                            ...
                            List<String> optsToAdd = new ArrayList<>();
                ...
                valueOptions = new ArrayList<>(); // to be populated
                                                        // dynamically
            }
            else if (source.equals("TaskCategory")) {
                List<String> codesWithNames = new ArrayList<>();
                ...
                // read-only
                StringBuilder syncedActivities = new StringBuilder();
                for (int i = 0; i < valueOptions.size(); i++) {
                    syncedActivities.append(valueOptions.get(i));
                    if (i < valueOptions.size() - 1)
                        syncedActivities.append("#");
                }
                activity.setAttribute("Synced Activities", syncedActivities.toString());
            }
    ...
    protected void createLabel(Composite parent, int verticalIndent) {
        ...
        int lStyle = wrapLabel ? SWT.WRAP : SWT.NONE;

        labelWidget = new Label(parent, lStyle);
        int lWidth = LABEL_WIDTH;
    ...
    private Text createTextInput(Composite parent) {
        ...
        if (isMultiLine())
            lStyle = lStyle | SWT.MULTI | SWT.WRAP | SWT.V_SCROLL;
        else
            lStyle = lStyle | SWT.SINGLE;

        final Text textField = new Text(parent, lStyle);
        if (font != null)
    ...
    private Combo createCombo(Composite parent) {
        ...
        int lStyle = SWT.DROP_DOWN | SWT.BORDER | this.style;
        if (isReadOnly())
            lStyle = lStyle | SWT.READ_ONLY;
        final Combo comboBox = new Combo(parent, lStyle);
        GridData gd = new GridData(SWT.LEFT);
        gd.horizontalSpan = span == 0 ? COLUMNS - 1 : span;
        gd.widthHint = getWidth();
        gd.horizontalIndent = indent;
    ...
    private ImageCombo createImageCombo(Composite parent) {
        ...
        int lStyle = SWT.BORDER | this.style;
        final ImageCombo imageCombo = new ImageCombo(parent, lStyle);
        GridData gd = new GridData(SWT.LEFT);
    ...
    private Link createLink(Composite parent) {
        ...
        int lStyle = SWT.SINGLE | this.style;
        Link link = new Link(parent, lStyle);
        if (font != null)
            ...
            public void widgetSelected(SelectionEvent e) {
                if (getValue() != null) {
                    if (getValue().startsWith("/MDWWeb/") || getValue().startsWith("/MDWHub/")
                            || getValue().startsWith("help")) {
                        try {
                            String docPath;
                            if (getValue().startsWith("help"))
                                docPath = Compatibility
                                .getDocumentationPath("/doc" + getValue().substring(4));
                            else
                                docPath = Compatibility
    ...
    private Label createInfo(Composite parent) {
        ...
        Label lLabel = new Label(parent, SWT.WRAP | style);
        GridData gd = new GridData(SWT.LEFT);
        gd.widthHint = getWidth();
        gd.horizontalIndent = indent;
        gd.horizontalSpan = 3;
        lLabel.setLayoutData(gd);
        if (getValue() != null)
            lLabel.setText(getValue());
        return lLabel;
    }
    ...
    private Button createButton(Composite parent) {
        String lLabel = comment == null ? "" : comment;
        labelWidget = new Label(parent, SWT.NONE);
        labelWidget.setText(lLabel);
    ...
    private Button createCheckbox(Composite parent) {
        ...
        checkbox.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                fireValueChanged(Boolean.valueOf(checked));
            }
        });
    ...
    private ListComposer createPickList(Composite parent) {
        ...
        int lStyle = SWT.DROP_DOWN | this.style;
        if (isReadOnly())
            lStyle = lStyle | SWT.READ_ONLY;
        final ListComposer pickList = new ListComposer(parent, lStyle, title, valueOptions, 200,
                isReadOnly());
        GridData gd = new GridData(SWT.LEFT);
        gd.horizontalSpan = COLUMNS - 1;
    ...
    public void fireValueChanged(Object newValue, boolean fireDirtyStateChanged) {
        Object lValue = newValue;
        if (newValue != null && valueConverter != null) {
            if (newValue instanceof String)
                lValue = valueConverter.toModelValue((String) newValue);
            else if (newValue instanceof List)
                lValue = ((List<?>) newValue).get(0).toString();
        }
        ...
        for (int i = 0; i < valueChangeListeners.getListeners().length; ++i) {
            ValueChangeListener listener = (ValueChangeListener) valueChangeListeners
                    .getListeners()[i];
            listener.propertyValueChanged(lValue);
        }
