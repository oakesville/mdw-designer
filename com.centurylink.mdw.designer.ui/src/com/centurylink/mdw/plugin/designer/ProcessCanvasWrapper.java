...
public class ProcessCanvasWrapper extends DesignerPanelWrapper implements AWTEventListener {
    private static final String MDW_EDITORS_PROCESS = "mdw.editors.process";
    private static final String OPEN_CALLING_INSTANCE = "Open Calling Instance";
    private static final String LAUNCH_PROCESS = "Launch Process";
    private static final String OPEN_SUBPROCESS = "Open SubProcess";
    ...
    private void handleSelection(MouseEvent mouseEvent) {
        ...
        if (mouseEvent.getID() == MouseEvent.MOUSE_PRESSED) {
            if (isInstance())
                obj = getProcessInstancePage().canvas.objectAt(lProcess, mouseX, mouseY,
                        getProcessInstancePage().canvas.getGraphics());
            else
                obj = getFlowchartPage().canvas.objectAt(lProcess, mouseX, mouseY,
                        getFlowchartPage().canvas.getGraphics());
        }
        else {
            obj = lProcess.objectAt(mouseX, mouseY,
                ...
                if (designerCanvasSelection instanceof WorkflowProcess) {
                    if (process.isInRuleSet()) {
                        ...
                        ImageDescriptor noteImageDesc = MdwPlugin
                                .getImageDescriptor(ICON_DOC_GIF);
            ...
            if (activity.isManualTask()) {
                        ...
                        public void widgetSelected(SelectionEvent e) {
                            ...
                            if (taskInstanceVO != null) {
                                ...
                                WebLaunchActions.getLaunchAction(activity.getProject(), webapp)
                                        .launch(activity.getProject(),
                                                taskInstParams + packageParam);
                            }
                        }
                    });
                }
            }
            else if (activity.isSubProcessInvoke() && activity.getSubProcessInstances() != null
                ...
                for (final ProcessInstanceVO subProcInst : activity.getSubProcessInstances()) {
                    ...
                    subProcItem.setImage(
                            MdwPlugin.getImageDescriptor(ICON_PROCESS_GIF).createImage());
                    subProcItem.addSelectionListener(new SelectionAdapter() {
                        @Override
                        public void widgetSelected(SelectionEvent e) {
        ...
        if (!isInstance() && designerCanvasSelection instanceof WorkflowProcess) {
            ...
            if (getProcess().getProject().isPureMdw52()) {
                ...
                ImageDescriptor callersImageDesc = MdwPlugin
                        .getImageDescriptor(ICON_PROCESS_GIF);
        ...
        try {
            pbi.busyWhile(new Runnable() {
                public void run() {
                    ...
                    if (subProcName == null) {
                        MessageDialog.openError(getDisplay().getActiveShell(), OPEN_SUBPROCESS,
                    ...
                    if (subProcess == null) {
                        PluginMessages.uiError(getDisplay().getActiveShell(),
                                ...
                                OPEN_SUBPROCESS, workflowProject);
                        return;
                    }
                    ...
                    IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                            .getActivePage();
                    try {
                        page.openEditor(subProcess, MDW_EDITORS_PROCESS);
                    }
                    catch (PartInitException ex) {
                        PluginMessages.uiError(getDisplay().getActiveShell(), ex, OPEN_SUBPROCESS,
                                workflowProject);
                    }
                }
            });
            ...
            PluginMessages.uiError(ex, OPEN_SUBPROCESS, getProject());
        }
    }
            ...
            pbi.busyWhile(new Runnable() {
                public void run() {
                    ...
                    IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                            .getActivePage();
                    try {
                        page.openEditor(processVersion, MDW_EDITORS_PROCESS);
                    }
                    catch (PartInitException ex) {
    ...
    public void launchProcess(final WorkflowProcess processVersion, final boolean debug) {
        ...
        try {
            ...
            PluginMessages.uiError(ex, LAUNCH_PROCESS, getProject());
        }
    }
                ...
                pbi.busyWhile(new Runnable() {
                    public void run() {
                        ...
                        try {
                            ...
                            PluginMessages.uiError(ex, OPEN_CALLING_INSTANCE, getProject());
                        }
                    }
                });
            }
