...
public class Importer {
    ...
    private static final String PARSING_XML = "Parsing XML";
    ...
    public WorkflowPackage importPackage(final WorkflowProject project, final String content,
        ...
        if (!emptyRuleSets.isEmpty()) {
            ...
            if (shell != null) {
                ...
                for (RuleSetVO rs : emptyRuleSets)
                    msg2.append("   ").append(rs.getLabel()).append("\n");
                PluginMessages.log(msg2.toString());
            }
        ...
        if (preexistingVersion > 0 && existingProcessVOs.isEmpty()
                && existingRuleSets.isEmpty()) {
            progressMonitor.subTask("Saving Package");
    ...
    public WorkflowProcess importProcess(final WorkflowPackage targetPackage,
            ...
            RemoteException, XmlException, ValidationException {
                ...
                for (Map.Entry<String, String> attrs : importedAttrs.entrySet()) {
                    if (!WorkAttributeConstant.isAttrNameFor(attrs.getKey(), attrPrefix))
                        throw new DataAccessException("Expected attribute prefix: " + attrPrefix
                                + " in attributes XML but found attribute: " + attrs.getKey());
                    else
                        setAttrs.put(attrs.getKey(), attrs.getValue());
                }
            }
    ...
    public void importTaskTemplates(final WorkflowPackage pkg, final String xml,
            ...
            throws XmlException, DataAccessException {
        progressMonitor.subTask(PARSING_XML);
