    ...
    private int saveDialog(boolean closeButton) {
        ...
        try {
            ...
            else // save the template
            {
                ...
                if ((pkgMeta != null && pkgMeta.trim().startsWith("{")) || taskTemplate.getPackage()
                        .getSchemaVersion() >= DataAccess.schemaVersion61) {
    ...
    class NoticesValueChangeListener extends AttributeValueChangeListener {
        ...
        public void attributeValueChanged(String newValue) {
            ...
            if (newValue.startsWith("[")) {
                try {
                    rows = StringHelper.parseTable(newValue, ',', ';', new JSONArray(newValue).length());
                }
                ...
                if (columnCount <= 3)
                    notifierCol = 2;
                rows = StringHelper.parseTable(newValue, ',', ';', columnCount);
            }
            boolean changed = false;
            for (String[] row : rows) {
                if (!StringHelper.isEmpty(row[1]) && StringHelper.isEmpty(row[notifierCol])) {
                    row[notifierCol] = TemplatedNotifier.getDefaultNotifierImpl(row[1]);
                    changed = true;
                }
