/*
6 * Copyright (C) 2017 CenturyLink, Inc.
 *
...
public class AutomatedTestSuite extends WorkflowElement {
    ...
    public AutomatedTestCase getPostmanTestCaseByMethod(List<AutomatedTestCase> cases, String method) {
        for (AutomatedTestCase testCase : cases)
            if (testCase.isPostman() && testCase.getMethod().equals(method))
                return testCase;
        return null;
    }

    public List<AutomatedTestCase> getPostmanTestCasesByName(String name) {
        List<AutomatedTestCase> lst = new ArrayList<>();
        for (AutomatedTestCase testCase : getTestCases())
            if (testCase.isPostman() && testCase.getName().equals(name))
                lst.add(testCase);
        return lst;
    }
    ...
    public void setRunning(boolean running) {
        ...
        if (running) {
            ...
            for (AutomatedTestCase regrTestCase : testCases) {
                ...
                else if (regrTestCase.isSuccess() && !suiteStatus.equals(TestCase.STATUS_ERROR)
    ...
    public boolean equals(Object o) {
        if (!(o instanceof AutomatedTestSuite))
            return false;
    ...
    public void writeTestCaseResultsXml(AutomatedTestCase exeTestCase) {
        ...
        for (AutomatedTestCase testCase : getProject().getTestCases()) {
            if (exeTestCase.getName().equals(testCase.getName()))
                getProject().fireTestCaseStatusChange(exeTestCase, exeTestCase.getStatus());
            ...
            if (pkg.getTestCases() != null && !pkg.getTestCases().isEmpty()) {
                ...
                for (AutomatedTestCase autoTestCase : pkg.getTestCases()) {
                    ...
                    if (autoTestCase.isPostman() && exeTestCase.isPostman())
                        testCase = testCaseList.getTestCaseByItemName(autoTestCase.getPath(),
                                exeTestCase.getMethod() + "/" + exeTestCase.getItemName());
                    else
                    ...
                    if (testCase == null) {
                        if (autoTestCase.getItems() != null) {
                            for (TestCaseItem item : autoTestCase.getItems()) {
                                com.centurylink.mdw.test.TestCase itemTestCase = new com.centurylink.mdw.test.TestCase(
                                        pkg.getName(), new AssetInfo(autoTestCase.getRawFile()));
                                String method = "";
                                ...
                                if (exeTestCase.getMethod() != null && exeTestCase.getMethod().equals(method)
                                        && exeTestCase.getItemName() != null && exeTestCase.getItemName().equals(item.getName()))
                    ...
                    if (testCase != null) {
                        ...
                        if (autoTestCase.getItems() != null && exeTestCase.getItemName() != null
                                && !(exeTestCase.getMethod() + "/" + exeTestCase.getItemName())
                                        .equals(testCase.getItemName()))
                            caseMatched = false;
                        if (exeTestCase.getPath().equals(autoTestCase.getPath()) && caseMatched) {
                            getProject().fireTestCaseStatusChange(exeTestCase,
    ...
    public void writeLoadTestResults(int totalPrepared, int totalCompleted,
            ...
            String finalEndTime, int totalProcessStarted, double speed) {
        StringBuilder suiteBuf = new StringBuilder();
