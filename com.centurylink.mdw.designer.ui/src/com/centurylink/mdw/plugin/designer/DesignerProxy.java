...
import com.centurylink.mdw.designer.auth.TokenExpiryException;
...
import com.centurylink.mdw.designer.auth.MdwAuthenticator;
...
import com.centurylink.mdw.designer.model.AppSummary;
import com.centurylink.mdw.designer.model.DbInfo;
...
public class DesignerProxy {
    ...
    private static final String ORA_02292 = "ORA-02292";
    private static final String DELETING = "Deleting '";
    private static final String SAVE_ATTRIBUTES = "Save Attributes";
    private static final String SERVER_OFFLINE= "Server appears to be offline: ";
    private static final String RENAME_ERROR = "Can't Rename";
    private static final String TO_PKG = "' to package '";
    private static final String FROM_PKG = "' from package '";
    ...
    public void initialize(ProgressMonitor progressMonitor) throws Exception {
        ...
        try {
            ...
            if (user != null && user.getJwtToken() != null)
                user.setJwtToken(((MdwAuthenticator) project.getAuthenticator())
                        .doAuthentication(user.getUsername(), user.getPassword()));

            if (user == null)
                handleLazyUserAuth();

            if (project.getPersistType() == WorkflowProject.PersistType.Git) {
                restfulServer = createRestfulServer(
                        project.getMdwDataSource().getJdbcUrlWithCredentials(),
                        project.getMdwMajorVersion() * 1000 + project.getMdwMinorVersion() * 100);
                    ...
                    try {
                        if (isGit) {
                            ...
                            Platform.getBundle("org.eclipse.egit.ui").start(); // avoid
                                                                               ...
                                                                               // login
                                                                               // fails
                            AppSummary appSummary = restfulServer
                                .getAppSummary();
                                ...
                                try {
                                    ...
                                    if (isGit)
                                        PluginMessages.uiMessage(
                                                "Extra/Archived packages not retrieved.  Showing only assets from Git.",
                                                "Load Workflow Project", PluginMessages.INFO_MESSAGE);
                                    else
                                        throw ex;
                                }
                            }
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                ...
                try {
                    ...
                    if (ex.getCause() != null && ex.getCause().getMessage() != null
                            && ex.getCause().getMessage().startsWith(ORA_02292)) {
                        PluginMessages.log(ex);
    ...
    public RunnerResult createNewWorkflowAsset(final WorkflowAsset asset,
            final boolean lockToUser) {
        String progressMsg = SAVING + asset.getName() + "'";
    ...
    public void saveWorkflowAsset(WorkflowAsset asset, boolean keepLocked)
        ...
        if (!asset.isBinary() &&  (asset.getContent() == null || asset.getContent().length() == 0))
    ...
    public boolean saveOverrideAttributes(String prefix, String ownerType, Long ownerId,
        ...
        try {
            ...
            catch (TokenExpiryException ex) {
                PluginMessages.log(ex);
                MessageDialog.openWarning(MdwPlugin.getShell(), SAVE_ATTRIBUTES,
                        "Authentication token has been expired. Please refresh process explorer view and try.");
                return false;
            }
            catch (DataAccessOfflineException ex) {
                PluginMessages.log(ex);
                MessageDialog.openWarning(MdwPlugin.getShell(), SAVE_ATTRIBUTES,
                        SERVER_OFFLINE + ex.getMessage());
                return false;
            }
            cacheRefresh.fireRefresh(false);
            return true;
            ...
            PluginMessages.uiError(ex, SAVE_ATTRIBUTES, project);
            return false;
        }
    }
    ...
    public void saveProcessAs(final WorkflowProcess processVersion,
            final WorkflowPackage targetPackage, final String newName) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                ...
                if (project.checkRequiredVersion(6, 1, 1))
                    newPackage.getPackageVO().setSchemaVersion(DataAccess.schemaVersion61);
                else if (project.checkRequiredVersion(6))
    ...
    public void renamePackage(final WorkflowPackage packageToRename, final String newName) {
        ...
        if (workflowProject.packageNameExists(newName)) {
            ...
            MessageDialog.openError(shell, RENAME_ERROR,
            ...
            public void perform()
                ...
                try {
                    ...
                    MdwPlugin.getDisplay().syncExec(new Runnable() {
                        public void run() {
                            String msg = "Cannot connect to server to check for instances (Authentication token has been expired. Please refresh process explorer view and try).  Are you sure you want to rename?";
            ...
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                if (!includeInstances) {
                    try {
                        ...
                        MdwPlugin.getDisplay().syncExec(new Runnable() {
                            public void run() {
                                String msg = "Cannot connect to server to check for instances (Authentication token has been expired. Please refresh process explorer view and try).  Are you sure you want to delete?";
                                confirm.append(MessageDialog.openConfirm(MdwPlugin.getShell(),
                                        "Delete Process", msg));
                            }
                        });
    ...
    public void savePackage(WorkflowPackage packageVersion, boolean newVersion)
        ...
        if (newVersion && project.checkRequiredVersion(6, 1, 1))
            packageVersion.getPackageVO().setSchemaVersion(DataAccess.schemaVersion61);
    ...
    public ProcessList getProcessInstanceList(WorkflowProcess processVersion,
        ...
        try {
            ...
            PluginMessages.log(ex);
            String msg = SERVER_OFFLINE + ex.getMessage();
            ...
            PluginMessages.log(ex);
            String msg = "Authentication token has been expired. Please refresh process explorer view and try.";
            MessageDialog.openWarning(MdwPlugin.getShell(), "Retrieve Process Instances", msg);
            return new ProcessList(ProcessList.PROCESS_INSTANCES,
                    new ArrayList<ProcessInstanceVO>());
        }
    ...
    public RunTimeDesignerCanvas loadProcessInstance(WorkflowProcess processVersion,
        ...
        try {
                                ...
                                for (ProcessInstanceVO embeddedProcInst : instSubGraph
                                    ...
                                    for (Map.Entry<Long, List<TaskInstanceVO>> embeddedTaskInst : embeddedTaskInsts.entrySet()) {
                                        if (taskInstances.get(embeddedTaskInst.getKey()) == null)
                                            taskInstances.put(embeddedTaskInst.getKey(),
                                                    new ArrayList<TaskInstanceVO>());
                                        taskInstances.get(embeddedTaskInst.getKey())
                                                .addAll(embeddedTaskInst.getValue());
                                    }
                                }
                            }
                        }
            ...
            PluginMessages.log(ex);
            MessageDialog.openWarning(MdwPlugin.getShell(), "Retrieve Process Instances",
                    "Authentication token has been expired. Please refresh process explorer view and try.");
        }
        catch (Exception ex) {
            PluginMessages.log(ex);
            ...
            if (activity.isHeterogeneousSubProcInvoke()) {
                ...
                if (procMapStr != null && !procMapStr.isEmpty()) {
                    ...
                    for (String[] row : procMap) {
                        ...
                        if (found != null) {
                            if (!processes.contains(found)) {
                                processes.add(found);
                                subProcessVO.setProcessId(found.getId());
                                subProcessVO.setProcessName(found.getName());
                                insts.addAll(dataAccess.getDesignerDataAccess()
                                        .getChildProcessInstance(parentProcessInstanceId,
                                                subProcessVO, parentProcess.getProcessVO()));
                            }
                        }
                        else {
    ...
    public void moveProcessToPackage(final String processName, final String version,
        ...
        String progressMsg = "Moving '" + processName + " v " + version + TO_PKG
    ...
    public void removeProcessFromPackage(final String processName, final String version,
        ...
        String progressMsg = "Removing '" + processName + " v " + version + FROM_PKG
    ...
    public void moveExternalEventToPackage(final Long externalEventId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Moving external event id '" + externalEventId + TO_PKG
                + targetPackage.getName() + "'";
    ...
    public void removeExternalEventFromPackage(final Long externalEventId,
            final WorkflowPackage packageVersion) {
        String progressMsg = "Removing external event id '" + externalEventId + FROM_PKG
                + packageVersion.getName() + "'";
    ...
    public void addActivityImplToPackage(final Long activityImplId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Adding activity implementor id '" + activityImplId + TO_PKG
                + targetPackage.getName() + "'";
    ...
    public Long moveWorkflowAssetToPackage(final Long assetId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Moving Asset ID: " + assetId + TO_PKG
                + targetPackage.getName() + "'";
    ...
    public void updateProcessesAndAssetsToLatest(final WorkflowPackage packageVersion) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                List<WorkflowProcess> pkgProcesses = new ArrayList<>();
                ...
                List<WorkflowAsset> pkgAssets = new ArrayList<>();
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (MdwPlugin.getSettings().isLogConnectErrors())
                            PluginMessages.log(ex);
                        try {
                            varTypeVO.setUpdateable(false);
                            if (!project.checkRequiredVersion(6, 1, 1)) {
                                String path = "DocumentValue?format=xml&id="
                                        + docRef.getDocumentId() + "&type="
                                        + varTypeVO.getVariableType();
                ...
                if (value != null) {
                    ...
                    if (isDoc) {
                        ...
                        if (docVo != null) {
                            ...
                            boolean isJavaObj = Object.class.getName()
                                    .equals(varInstInfo.getType());
                            if (isJavaObj) {
                                try {
                                    value = new JavaObjectTranslator().realToObject(value)
                                            .toString();
                                }
                                catch (TranslationException ex) {
                                    if (MdwPlugin.getSettings().isLogConnectErrors())
                                    ...
                                    if (tryServer) {
                                        try {
                                            String path = "DocumentValues/"
                                                    + docRef.getDocumentId();
                                            if (!project.checkRequiredVersion(6, 1, 1))
                                                path = "DocumentValue?id=" + docRef.getDocumentId()
                                                        + "&type=" + varInstInfo.getType()
                                                        + "&format=text";
    ...
    public AppSummary retrieveAppSummary(boolean failSilently) {
        try {
            return restfulServer.getAppSummary();
        }
        catch (Exception ex) {
            if (!failSilently)
    ...
    public String sendExternalEvent(String request, Map<String, String> headers)
        ...
        String ret = dataAccess.getDesignerDataAccess().sendMessage("RestfulWebService", request,
                headers);
        dataAccess.auditLog(Action.Send, Entity.Event, Long.valueOf(0), "External Event");
        return ret;
    }
    ...
    public String notifyProcess(String request, Map<String, String> headers)
        ...
        String ret = dataAccess.getDesignerDataAccess().sendMessage("RestfulWebService", request,
                headers);
        dataAccess.auditLog(Action.Send, Entity.Event, Long.valueOf(0), "Notify Process");
        return ret;
    }
    ...
    public void remoteImportVcs() throws DataAccessException {
        try {
            dataAccess.getDesignerDataAccess().importFromVcs(project.getServiceUrl());
        }
        catch (Exception ex) {
    ...
    public String checkForServerDbMismatch() {
        AppSummary appSummary = retrieveAppSummary(true);
        if (appSummary == null)
            return SERVER_OFFLINE + restfulServer.getMdwWebUrl();
        else {
            if (project.isFilePersist())
    ...
    public String checkForDbMismatch(AppSummary appSummary) {
    ...
    private RestfulServer createRestfulServer(String jdbcUrl, int schemaVersion) {
        if (project.isRemote() && project.checkRequiredVersion(6, 0, 13))
            if (project.getUser().getJwtToken() != null)
                return new RestfulServer(jdbcUrl, project.getUser().getUsername(),
                        project.getUser().getJwtToken(), project.getServiceUrl(), schemaVersion);
            else
                return new RestfulServer(jdbcUrl, project.getUser().getUsername(),
                         project.getUser().getPassword(), project.getServiceUrl(), project.getAppSummaryUrl(), schemaVersion);
        else
            return new RestfulServer(jdbcUrl, project.getUser().getUsername(),
