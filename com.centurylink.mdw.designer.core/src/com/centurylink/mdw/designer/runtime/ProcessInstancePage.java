...
public class ProcessInstancePage extends DesignerPage implements ActionListener, MouseListener {
    private static final String ACTION_ZOOM = "ZOOM";
    private static final String SAVE_AS_JPEG= "Save ProcessInstance as JPEG";
    ...
    public ProcessInstancePage(MainFrame frame) {
        ...
        JSplitPane eastPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, treeScrollPane,
                nodeScrollpane);
        ...
        JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, eastPane,
                canvasScrollpane);
    ...
    public void createToolBar() {
        ...
        if (frame.getStartPage() != null) {
            backButton = createToolButton("back", "back.gif", "Back to last page", Constants.BACK,
                    this);
            createToolButton("Save", "save.gif", SAVE_AS_JPEG,
                    Constants.ACTION_SAVE, this);
            createToolButton("Variables", "variable.jpg", "variables", Constants.ACTION_DOCVIEW,
                    this);
        }
        else {
            backButton = createToolButton("back", "back.gif", "Back to last page", Constants.BACK,
                    this);
            createToolButton("Process Definition", "process.jpg", "process definition",
                    Constants.ACTION_FLOWVIEW, this);
            if (!frame.dao.noDatabase())
                createToolButton("ProcessInstance", "table24.gif",
                        "All Process Instance for this process",
                        Constants.VIEW_ALL_PROCESS_INSTANCE, this);
            createToolButton("Save", "save.gif", SAVE_AS_JPEG,
                    Constants.ACTION_SAVE, this);
            createToolButton("Variables", "variable.jpg", "variables", Constants.ACTION_DOCVIEW,
                    this);
            createToolButton("Send message", "start.gif", "Send message", Constants.ACTION_START,
                    this);
            createToolButton("Remove", "delete.gif", "remove instance from menu",
                    Constants.ACTION_DELETE, this);

            // AK..added 02/25/2011
            createToolButton("Logout", "logout.gif", "Log out", Constants.ACTION_LOGOUT, this);

            String[] actions = { Constants.ACTION_PROCLIST, Constants.ACTION_PACKAGE, ACTION_SCRIPT,
                    ACTION_TESTING };
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (cmd.equals(Constants.ACTION_SAVE)) {
            try {
                FileDialog fd = new FileDialog(frame, SAVE_AS_JPEG,
                        FileDialog.SAVE);
    ...
    public String getTitle() {
        return frame.getDesignerTitle() + " - " + processInstance.getProcessName() + " instance "
                + processInstance.getId() + " [" + processInstance.getMasterRequestId() + "]";
    }
