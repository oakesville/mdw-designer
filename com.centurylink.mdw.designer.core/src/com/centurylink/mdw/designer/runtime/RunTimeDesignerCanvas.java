...
public class RunTimeDesignerCanvas extends CanvasCommon
        implements ImageObserver, MouseListener, MouseMotionListener, KeyListener {
        ...
        if (obj != null && obj instanceof Node) {
            ...
            if (insts != null) {
                ...
                for (ActivityInstanceVO one : node.getInstances()) {
                    sb.append(one.getId()).append(": ").append(one.getStartDate().toString())
                            .append("<p>");
                    k++;
                    if (k >= 5)
                        break;
