...
// writer.print(word);
    ...
    public int firstDiffLine(TestFile expected, Map<String, String> map, MbengDocument refdoc,
            PrintStream log) {
        int i = 0;
        int n = lines.size();
        TestFileLine myLine = null;
        for (TestFileLine itsLine : expected.lines) {
            if (itsLine.getCommand().startsWith("#"))
                continue;
            if (i >= n) {
                int lineNumber =  myLine != null ? myLine.getLineNumber() : 0;
                return (n == 0 ? 1 : lineNumber);
            }
            myLine = lines.get(i++);
        ...
        while (i < n) {
            myLine = lines.get(i++);
            if (!myLine.getCommand().startsWith("#"))
                return myLine.getLineNumber();
        }
        return -1;
    }
