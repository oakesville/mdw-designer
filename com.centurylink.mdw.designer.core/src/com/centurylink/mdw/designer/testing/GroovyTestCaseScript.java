...
public abstract class GroovyTestCaseScript extends Script {
        ...
        if (getBinding().hasVariable("masterRequestId")) {
            ...
            if (!run.getMasterRequestId().equals(defined)) {
                ...
                if (run.getMasterRequestListener() != null)
                    run.getMasterRequestListener().syncMasterRequestId(old,
                            run.getMasterRequestId());
                if (run.verbose)
    ...
    // server-side placeholders: simply return server-side syntax for
    ...
    public ApiRequest request(String target, Map<String,String> options, Closure<?> cl) throws TestException {
        ...
        try {
            ...
            apiTestCase.setResultDirectory(new File(
                    resultsPath.substring(0, resultsPath.indexOf("\\assets\\")) + "\\testResults\\" + ruleSetVO.getPackageName()));
    ...
    public ApiResponse submit(ApiRequest apiTest) throws TestException {
        ...
        if (testItem.getOption("caseName") == null)
            testItem.setOption("caseName", getTestCase().getName());
    ...
    public TestCaseProcess[] processes(String... targets) throws TestException {
        ...
        for (int i = 0; i < targets.length; i++) {
            try {
                ...
                if (i == 0) // by convention > 0 are subprocesses where
    ...
    public TestCaseAsset getDefaultExpectedResults(TestCaseProcess process) throws TestException {
        ...
        String resultsAssetName = testAssetName.substring(0, testAssetName.lastIndexOf('.'))
    ...
    public Verifiable verify(ApiResponse response, Closure<?> cl) throws TestException {
        ...
        try {
            ...
            if (item.getOption("caseName") == null)
                item.setOption("caseName", getTestCase().getName());
            ...
            item.setStatus(TestCase.STATUS_STOP);
    ...
    public TestCaseMessage message(String protocol, Closure<?> cl) throws TestException {
        TestCaseMessage message = protocol == null ? new TestCaseMessage()
                : new TestCaseMessage(protocol);
        if (cl != null) {
            cl.setResolveStrategy(Closure.DELEGATE_FIRST);
            cl.setDelegate(message);
            cl.call();
    ...
    public TestCaseEvent notify(TestCaseEvent event) throws TestException {
        event.setMessage(substitute(event.getMessage()));
    ...
    public TestCaseAdapterStub endpoint(Closure<Boolean> matcher, Closure<?> init)
            throws TestException {
        return endpoint(matcher, null, init);
    }

    public TestCaseAdapterStub endpoint(Closure<Boolean> matcher, Closure<String> responder,
            Closure<?> init) throws TestException {
        TestCaseAdapterStub adapterStub = adapter(matcher, null, init);
        adapterStub.setEndpoint(true);
        return adapterStub;
    }

    public TestCaseAdapterStub adapter(Closure<Boolean> matcher, Closure<?> init)
            throws TestException {
     ...
     */
    public TestCaseAdapterStub adapter(Closure<Boolean> matcher, Closure<String> responder,
            Closure<?> init) throws TestException {
                    ...
                    if (adapterStub.getResponse().indexOf("${") >= 0) {
                        try {
                            ...
                            if (request.toString().startsWith("{")) {
                                ...
                                GPathResult gpathRequest = new XmlSlurper()
                                        .parseText(request.toString());
                            ...
                            GroovyShell shell = new GroovyShell(
                                    GroovyTestCaseScript.class.getClassLoader(), binding,
                                    compilerCfg);
                            shell.setProperty("out", testCaseRun.log);
                            DelegatingScript script = (DelegatingScript) shell
                                    .parse("return \"\"\"" + adapterStub.getResponse() + "\"\"\"");
                            script.setDelegate(GroovyTestCaseScript.this);
                            return script.run().toString();
                        }
                        catch (Exception ex) {
                            getTestCaseRun().log.println(
                                    "Cannot perform stub substitutions for request: " + request);
    ...
    public TestCaseActivityStub activity(Closure<Boolean> matcher, Closure<String> completer)
            throws TestException {
        return new TestCaseActivityStub(matcher, completer);
    }

    public TestCaseActivityStub activity(final String match, Closure<String> completer)
            throws TestException {
        ...
        return new Closure<Boolean>(this, this) {
            ...
            public Boolean call(Object request) {
                try {
                    ...
                    getTestCaseRun().log
                            .println("Failed to parse request as XML/JSON. Stub response: "
    ...
    protected String substitute(String before) {
        ...
        CompilerConfiguration compilerCfg = new CompilerConfiguration();
        compilerCfg.setScriptBaseClass(DelegatingScript.class.getName());
        GroovyShell shell = new GroovyShell(GroovyTestCaseScript.class.getClassLoader(),
                getBinding(), compilerCfg);
        DelegatingScript script = (DelegatingScript) shell
                .parse("return \"\"\"" + before + "\"\"\"");
        script.setDelegate(GroovyTestCaseScript.this);
