...
public class AutoTestAntTask extends MatchingTask {
    ...
    private Map<String, String> testCaseStatuses = new HashMap<>();
    private Map<String, TestCaseRun> masterRequestRunMap = new HashMap<>();
    ...
    public String getDbAssetTests() {
        return dbAssetTests;
    }

    public void setDbAssetTests(String tests) {
        this.dbAssetTests = tests;
    }
    ...
    public void setWorkflowDir(File dir) {
    ...
    public void setTestResultsDir(File dir) {
        this.testResultsDir = dir;
    }

    public File getTestResultsSummaryFile() {
        return testResultsSummaryFile;
    }

    public void setTestResultsSummaryFile(File file) {
        this.testResultsSummaryFile = file;
    }
    ...
    public boolean isLoadTests() {
        return loadTests;
    }

    public void setLoadTests(boolean loadTests) {
    ...
    public void setOldNamespaces(boolean oldNamespaces) {
    ...
    public void setFailOnFirstError(boolean fofe) {
        this.failOnFirstError = fofe;
    }

    public URL getTestResultsBaseUrl() {
        return testResultsBaseUrl;
    }

    public void setTestResultsBaseUrl(URL testResultsBaseUrl) {
        this.testResultsBaseUrl = testResultsBaseUrl;
    }
    ...
    public void setSslTrustStore(File trustStore) {
    ...
    public void execute() throws BuildException {
        try {
            ...
            RestfulServer restfulServer = new RestfulServer(jdbcUrl == null ? "dummy" : jdbcUrl,
            ...
            if (workflowDir != null) {
                ...
                designerDataAccess = new DesignerDataAccess(restfulServer, null, user,
                        oldNamespaces);
                if (designerDataAccess.getUser(user) == null)
                    throw new BuildException("User '" + user
                            + "' not found to run automated tests for this environment.");
                if (!designerDataAccess.getUser(user).hasRole(UserGroupVO.COMMON_GROUP,
                        UserRoleVO.PROCESS_EXECUTION)
                        && !designerDataAccess.getUser(user).hasRole(UserGroupVO.SITE_ADMIN_GROUP,
                                UserRoleVO.PROCESS_EXECUTION))
                    throw new BuildException("User '" + user
                            + "' not authorized to run automated tests for this environment.");
            }
                    ...
                    Stubber stubber = new Stubber() {
                        public String processMessage(String masterRequestId, String request) {
                            try {
                                    ...
                                    if (request != null && request.trim().startsWith("{")) {
                                        ...
                                        if (requestJson != null) {
                                            if (requestJson.has(ActivityStubRequest.JSON_NAME)) {
                                                activityStubRequest = new ActivityStubRequest(
                                                        requestJson);
                                                activityRuntimeContext = activityStubRequest
                                                        .getRuntimeContext();
                                            }
                                            else if (requestJson.has("ActivityRuntimeContext")) {
                                                activityRuntimeContext = new ActivityRuntimeContext(
                                                        requestJson);
                                            }
                                            else if (requestJson
                                                    .has(AdapterStubRequest.JSON_NAME)) {
                                                adapterStubRequest = new AdapterStubRequest(
                                                        requestJson);
                                            }
                                        }
                                    }
                                ...
                                return run.getStubResponse(masterRequestId, request,
                                        run.getRunNumber());
                            }
                            catch (Exception ex) {
    ...
    public void runTests() throws Exception {
        ...
        if (workflowDir != null) {
            ...
            String jsonString;
            if (testResultsSummaryFile.exists()
                    && testResultsSummaryFile.getName().endsWith(".json")) {
            ...
            for (String caseFilePath : caseFilePaths) {
                ...
                pkgTests.getTestCases().add(
                        new com.centurylink.mdw.test.TestCase(pkgName, new AssetInfo(caseFile)));
            ...
            for (String asset : assets) {
                ...
                String language = RuleSetVO
                        .getLanguage(assetName.substring(assetName.lastIndexOf('.')));
        ...
        if (hasGherkin && hasNonGherkin)
            throw new BuildException(
                    "Gherkin/non-Gherkin tests require separate task/target executions.");

        boolean useStdErr = System.getProperty("org.gradle.appname") != null; // gradle
                                                                              // does
                                                                              // not
                                                                              // show
                                                                              // output
        ...
        LogMessageMonitor monitor = hasGherkin ? null
                : new LogMessageMonitor(designerDataAccess, oldNamespaces);
        if (monitor != null)
            monitor.start(true);

        for (TestCase testCase : testCases) {
            String masterRequestId = user + "-"
                ...
                if (testCase.getMasterRequestId() != null) {
                    if (testCase.getMasterRequestId().indexOf("${masterRequestId}") != -1) {
                        masterRequestId = testCase.getMasterRequestId()
                                .replace("${masterRequestId}", masterRequestId);
                    }
                    else
                        masterRequestId = testCase.getMasterRequestId();
                }
                testCase.setMasterRequestId(masterRequestId);
                run = new GherkinTestCaseRun(testCase, 0, masterRequestId,
                        new DesignerDataAccess(designerDataAccess), monitor, processCache,
                        oldNamespaces, resultDir, stubbing, stubPort);
            }
            else if (testCase.isGroovy())
                run = new GroovyTestCaseRun(testCase, 0, masterRequestId,
                        new DesignerDataAccess(designerDataAccess), monitor, processCache,
                        loadTests, true, oldNamespaces, null);
            else
                run = new TestCaseRun(testCase, 0, masterRequestId,
                        new DesignerDataAccess(designerDataAccess), monitor, processCache,
                        loadTests, true, oldNamespaces);
            ...
            if (!executeLog.getParentFile().exists() && !executeLog.getParentFile().mkdirs())
                throw new IOException(
                        "Unable to create test run directory: " + executeLog.getParentFile());
            PrintStream log = verbose
                    ? new TeePrintStream(useStdErr ? System.err : System.out, executeLog)
                    : new PrintStream(executeLog);
            run.prepareTest(false, resultDir, true, singleServer, stubbing, log);
            if (verbose)
                log("Test case prepared: " + testCase.getCaseName(),
                        useStdErr ? Project.MSG_ERR : Project.MSG_INFO);
    ...
    private synchronized void updateResults(boolean done) throws IOException {
        ...
        for (TestCase testCase : testCases) {
            String oldStatus = testCaseStatuses
                    .get(testCase.getPrefix() + "/" + testCase.getCaseName());
            boolean statusChanged = oldStatus == null || !oldStatus.equals(testCase.getStatus());
            testCaseStatuses.put(testCase.getPrefix() + "/" + testCase.getCaseName(),
                    testCase.getStatus());
            ...
            if (isErrored(testCase)) {
                if (statusChanged) {
                    ...
                    if (testResultsBaseUrl != null)
                        log("   ---> " + testResultsBaseUrl + "/" + testCase.getPrefix() + "/"
                                + testCase.getCaseName() + ".log", Project.MSG_ERR);

                }
                erroredCases++;
                if (failOnFirstError) {
                    buildException = new BuildException("Test case errored: " + testCase.getPrefix()
                ...
                if (statusChanged) {
                    log("Test case failed: " + testCase.getPrefix() + "/" + testCase.getCaseName(),
                            Project.MSG_ERR);
                    log("   ---> " + testCase.getMessage(), Project.MSG_ERR);
                    if (testResultsBaseUrl != null)
                        log("   ---> " + testResultsBaseUrl + "/" + testCase.getPrefix() + "/"
                ...
                failedCases++;
                if (failOnFirstError) {
                    buildException = new BuildException("Test case failed: " + testCase.getPrefix()
                            + "/" + testCase.getCaseName());
                    break;
                }
            }
            if (isFinished(testCase)) {
                if (statusChanged && !isErrored(testCase) && !isFailed(testCase))
                    log("Test case passed: " + testCase.getPrefix() + "/" + testCase.getCaseName(),
                            Project.MSG_ERR);
        ...
        if (done || buildException != null) {
            log("completed: " + completedCases + "   errored: " + erroredCases + "   failed: "
                    + failedCases, Project.MSG_ERR);
