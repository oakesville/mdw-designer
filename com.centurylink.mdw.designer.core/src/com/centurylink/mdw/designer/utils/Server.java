...
public class Server {
    ...
    public Server(String databaseUrl, String user, String password, boolean token, int schemaVersion) {
        this.databaseUrl = databaseUrl;
        this.user = user;
        this.schemaVersion = schemaVersion;
        if (token)
            this.jwtToken = password;
        else
    ...
    public DesignerHttpHelper getHttpHelper(String url) throws MalformedURLException {
        if (jwtToken != null)
            return new DesignerHttpHelper(new URL(url), jwtToken);
        if (url.startsWith("https") && password != null)
            return new DesignerHttpHelper(new URL(url), user, password);
        return new DesignerHttpHelper(new URL(url));
    }

    public boolean isSchemaVersion61() {
        return schemaVersion >= DataAccess.schemaVersion61;
    }
}
