...
public class RestfulServer extends Server {
    ...
    private String appSummaryUrl;

    public RestfulServer(String database, String user, String password, String mdwWebUrl, int schemaVersion) {
        super(database, user, password, true, schemaVersion);
        setMdwWebUrl(mdwWebUrl);
        this.appSummaryUrl = mdwWebUrl;
    }

    public RestfulServer(String database, String user, String password, String mdwWebUrl, String appSummaryUrl, int schemaVersion) {
        super(database, user, password, false, schemaVersion);
        setMdwWebUrl(mdwWebUrl);
        this.appSummaryUrl = appSummaryUrl;
    }
    ...
    public RestfulServer(RestfulServer cloneFrom) {
        super(cloneFrom);
        this.appSummaryUrl = cloneFrom.appSummaryUrl;
    }
    ...
    throws DataAccessException, XmlException, JSONException, IOException {
        if (isSchemaVersion61()) {
            ...
            ActionRequestDocument actionRequestDoc = getLaunchProcessBaseDoc(processId,
                    masterRequestId, owner, ownerId);

            for (VariableVO variableVO : variables.keySet()) {
                Parameter parameter = actionRequestDoc.getActionRequest().getAction()
                        .addNewParameter();
    ...
    public String launchSynchronousProcess(Long processId, String masterRequestId, String owner,
            Long ownerId, Map<VariableVO, String> variables, String responseVarName,
            boolean oldFormat)    throws XmlException, JSONException, IOException {
        if (isSchemaVersion61()) {
            ...
            Parameter responseVarParam = actionRequestDoc.getActionRequest().getAction()
                    .addNewParameter();
            ...
            try {
                ...
                String request;
                if (oldFormat)
                    request = DesignerCompatibility.getInstance()
                            .getOldActionRequest(actionRequestDoc);
                else
                    request = actionRequestDoc.xmlText(getXmlOptions());
    ...
    public String launchProcess(Long processId, String masterRequestId, String owner, Long ownerId,
            Map<VariableVO, String> variables) throws JSONException, IOException {
        ProcessRequest proc = new ProcessRequest(processId, masterRequestId, owner, ownerId,
                variables);
        String content = proc.getJson().toString();
        String path = "Processes/run/" + processId;
    ...
    public String retryActivityInstance(Long activityId, Long activityInstanceId, boolean oldFormat)
        ...
        if (isSchemaVersion61()) {
            return retryActivityInstance(activityInstanceId,
                    ActivityResultCodeConstant.RESULT_RETRY);
        }
        else {
            String request = buildRetryActivityInstanceRequest(activityId, activityInstanceId,
                    oldFormat);
    ...
    public String skipActivityInstance(Long activityId, Long activityInstanceId,
            String completionCode, boolean oldFormat)
            throws DataAccessException, XmlException, IOException {
        if (isSchemaVersion61()) {
            return skipActivityInstance(activityInstanceId, ActivityResultCodeConstant.RESULT_PROCEED,
                    completionCode);
        }
        else {
            String request = buildSkipActivityInstanceRequest(activityId, activityInstanceId,
                    completionCode, oldFormat);
            MDWStatusMessageDocument statusMsgDoc = invokeService(request);
            if (statusMsgDoc.getMDWStatusMessage().getStatusCode() == 0)
                return ""; // indicates success
            else
    ...
    public String retryActivityInstance(Long activityInstanceId, String action) throws IOException {
        String path = "Activities/" + activityInstanceId + "/" + action;
        return post(path, "{}");
    }

    public String skipActivityInstance(Long activityInstanceId, String action,
            String completionCode) throws IOException {
        String path = "Activities/" + activityInstanceId + "/" + action;
    ...
    public MDWStatusMessageDocument refreshCache(String path, String cacheName, boolean global, boolean includeDynamicJava) throws IOException {
        if (cacheName != null)
            path += "/" + cacheName;
        String content = "{distributed:" + global;
        if (!includeDynamicJava)
            content += ", excludeFormats:[" + RuleSetVO.JAVA + "]";
        content += "}";
        post(path, content);
    ...
    public MDWStatusMessageDocument invokeService(String request)
        ...
        try {
            return new AppSummary(new JSONObject(httpHelper.get()));
             ...
             throw new IOException(
                        "Unable to get app summary: " + url);
        }
    }

    public String invokeResourceService(String path)
