...
public abstract class DesignerPage extends JPanel implements ActionListener {
    ...
    protected static final String SAVE_RESOURCE = "Save Resource";
    ...
    public JButton createToolButton(String name, String iconname, String tip, String action,
            ActionListener listener) {
        JButton button;
        if (iconname != null) {
            button = new JButton(new ImageIcon(
                    this.getClass().getClassLoader().getResource("images/" + iconname)));
        }
        else
        ...
        if (name != null && name.equalsIgnoreCase("Delete"))
            button.setEnabled(true);
        toolbar.add(button);
        return button;
    }

    public JMenuItem createMenuItem(JMenu menu, String label, String action,
            ActionListener listener) {
    ...
    public void createToolRadio(ButtonGroup group, String label, String iconname, boolean selected,
        ...
        if (iconname != null) {
            ...
            button.setSelectedIcon(new ImageIcon(
                    this.getClass().getClassLoader().getResource("images/selected_" + iconname)));
        }
        else
    ...
    public JComboBox createToolDropdown(String[] choices, String action, ActionListener listener,
    ...
    public JComboBox createToolDropdown(String[] choices, String[] images, String tip,
        ...
        for (int i = 0; i < images.length; i++) {
            icon = new ImageIcon(
                    this.getClass().getClassLoader().getResource("images/" + images[i]));
            jcb.addItem(icon);
    ...
    public JCheckBox createToolCheckBox(String label, boolean selected, String action,
            ActionListener listener) {
        JCheckBox button;
    ...
    private void collectErrorMessages(Throwable e, List<String> msgs, int level) {
        ...
        if (!msgs.contains(msg))
            msgs.add(msg);
        if (level < 4 && e.getCause() != null)
            collectErrorMessages(e.getCause(), msgs, level + 1);
    }
    ...
    public void showError(Component parent, String msg, Throwable e) {
        List<String> msgs = new ArrayList<>(6);
        if (msg != null)
            msgs.add(msg);
     ...
     */
    private String saveProcess(Graph graph, PersistType persistType, int version, boolean lock)
    ...
    private void setProcessVersion(Node node, boolean includeId) {
        ...
        if (procname != null) {
            ...
            if (procdef != null) {
                node.setAttribute(WorkAttributeConstant.PROCESS_VERSION,
                        Integer.toString(procdef.getVersion()));
                if (includeId)
     ...
     */
    public Graph saveProcess(Graph process, MainFrame frame, PersistType persistType, int version,
            boolean lock) throws ValidationException, DataAccessException, RemoteException {
    ...
    public void showProcess(Graph process) {
        ...
        for (String action : actions) {
            if (action.equals(Constants.ACTION_PROCLIST)) {
                ...
                if (model.getDatabaseSchemaVersion() >= DataAccess.schemaVersion4
                    ...
                    createToolButton(null, "package.gif", "Packages", Constants.ACTION_PACKAGE,
                            this);
            }
            else if (action.equals(ACTION_TESTING)) {
                if (model.getDatabaseSchemaVersion() >= DataAccess.schemaVersion4) {
                    createToolButton(null, "testing.gif", "Regression test cases", ACTION_TESTING,
                            this);
                }
            }
                ...
                if (model.getDatabaseSchemaVersion() >= DataAccess.schemaVersion4)
                    createToolButton(null, "notice.gif", "Send external message to engine",
                            Constants.ACTION_START, this);
            }
            else if (action.equals(Constants.ACTION_NEW_ACTIVITY_IMPL)) {
                createToolButton(null, "new-activity.gif", "Activity Implementor",
                        Constants.ACTION_NEW_ACTIVITY_IMPL, this);
            }
            else if (action.equals(Constants.EXTERNAL_EVENT_BUTTON)) {
                createToolButton(null, "event.gif", "External Event Handlers",
                        Constants.EXTERNAL_EVENT_BUTTON, this);
            }
        }
    }
        ...
        try {
            ...
            if (!lock) {
                if (getConfirmation("You will lose your changes by unlock; are you sure?")) {
                    String errmsg = frame.dao.lockUnlockRuleSet(ruleset.getId(), frame.getCuid(),
                            false);
                    if (errmsg != null) {
                        changed = false;
     ...
     */
    public long saveResource(RuleSetVO ruleset, String newName) {
        ...
        if (isNew) {
            ...
            int option = frame.getOptionPane().choose(this, SAVE_RESOURCE, choices);
            keeplock = option == 2;
            asNewVersion = false;
            cancel = option == 1;
        }
        ...
        Long id = saveResourceSub(ruleset, asNewVersion, keeplock);
        if (id != null) {
    ...
    public Long saveResourceSub(RuleSetVO ruleset, boolean asNewVersion,
            boolean keeplock) {
        try {
            if (asNewVersion) {
                ...
                ruleset.setModifyingUser(null); // not saved to database, but
                                                // should be ok
                ruleset = newRuleSet;
            }
    ...
    public boolean serverIsRunning(boolean checkonly) {
        ...
        if (currentServer.getServerUrl() != null) {
            ...
            String serverUrl = frame.getOptionPane().getInput(this,
    ...
    public boolean verifyRoleForSite(String role) {
        if (model.getDatabaseSchemaVersion() >= DataAccess.schemaVersion52) {
            if (!model.userHasRole(UserGroupVO.SITE_ADMIN_GROUP, role)) {
                showError("You need to be in " + UserGroupVO.SITE_ADMIN_GROUP + " with " + role
            ...
            if (!model.userHasRole(UserGroupVO.COMMON_GROUP, role)) {
                showError("You need to have " + role + " role to perform this function");
                return false;
            }
            else
                return true;
