...
/**
 * Command-line importer for XML-formatted workflow asset packages. Much
 * duplication from Designer plug-in importer. Needs to be refactored but will
...
public class Importer {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 4 && args.length != 5) {
            System.err.println(
                    "arguments: <user> <password> <jdbcUrl|fileBasedRootDir> <xmlFile|xmlFileUrl> <overwrite=(true|FALSE)>");
        ...
        boolean overwrite = (args.length == 5 && "overwrite=true".equalsIgnoreCase(args[4])) ? true
                : false;

        try {
            DesignerDataAccess.getAuthenticator().authenticate(user, password);
            boolean local = !arg2.startsWith("jdbc:");
            RestfulServer restfulServer = new RestfulServer(local ? "jdbc://dummy" : arg2, user,
                    "http://dummy");
            ...
            if (local) {
                ...
                if (!userVO.hasRole(UserGroupVO.COMMON_GROUP, UserRoleVO.PROCESS_DESIGN))
                    throw new DataAccessException("User: '" + user + "' not authorized for "
            ...
            System.out.println(
                    "Importing with arguments: " + user + " ******* " + arg2 + " " + xmlFile);
    ...
    public void importPackage(String xml, boolean overwrite)
        ...
        if (existing != null) {
            if (existing.getVersion() == importedPackageVO.getVersion()) {
                String msg = "Target already contains Package '"
                        + importedPackageVO.getPackageName() + "' v"
                        + importedPackageVO.getVersionString();
                if (!overwrite)
                    throw new ActionCancelledException(
                            msg + " and overwrite argument NOT specified.");
                else
        ...
        for (ProcessVO importedProcessVO : importedPackageVO.getProcesses()) {
            ...
            try {
                existingProcess = dataAccess.getProcessDefinition(
            ...
            if (existingProcess != null) {
                ...
                if (dataAccess.getSupportedSchemaVersion() >= DataAccess.schemaVersion52) {
                    // content comparison
                    if (exporter == null) {
                        boolean isOldNamespaces = dataAccess
                                .getDatabaseSchemaVersion() < DataAccess.schemaVersion55;
                        exporter = DataAccess.getProcessExporter(
                                dataAccess.getDatabaseSchemaVersion(),
                                isOldNamespaces ? DesignerCompatibility.getInstance() : null);
                    }
                    String existingProcessXml = dataAccess.getRuleSet(existingProcess.getId())
                            .getRuleSet();
                    String importedProcessXml = exporter.exportProcess(importedProcessVO,
                            dataAccess.getDatabaseSchemaVersion(), null);
            ...
            for (ProcessVO subProcVO : importedProcessVO.getSubProcesses()) {
                ...
                try {
                    existingSubProc = dataAccess.getProcessDefinition(subProcVO.getProcessName(),
                            subProcVO.getVersion());
                }
                catch (DataAccessException ex) {
                    // trap process does not exist
                    if (!ex.getMessage().startsWith("Process does not exist;"))
        ...
        if (importedPackageVO.getRuleSets() != null) {
            ...
            for (RuleSetVO importedRuleSet : importedPackageVO.getRuleSets()) {
                ...
                if (dataAccess.getSupportedSchemaVersion() >= DataAccess.schemaVersion55) {
                    ...
                    if (existing != null) {
                        RuleSetVO latestAsset = dataAccess.getRuleSet(existing.getPackageId(),
                                importedRuleSet.getName());
                        if (latestAsset != null
                                && latestAsset.getVersion() >= importedRuleSet.getVersion())
                            existingAsset = latestAsset;
                    }
                }
                else {
                    existingAsset = dataAccess.getRuleSet(importedRuleSet.getName(),
        ...
        if (existing != null && importedPackageVO.getTaskTemplates() != null
        ...
        if (!conflicts.isEmpty()) {
            ...
            if (isLocal())
                msg = "The following versions exist locally in '"
                        + importedPackageVO.getPackageName() + "' and will be overwritten:";
            else
        ...
        if (!emptyRuleSets.isEmpty()) {
            System.out.println(
                    "The following assets from package '" + importedPackageVO.getPackageName()
        ...
        if (importedPackageVO.getProcesses() != null) {
            ...
            nodeMetaInfo.init(dataAccess.getActivityImplementors(),
                    dataAccess.getDatabaseSchemaVersion());
        ...
        if (importedPackageVO.getProcesses() != null) {
            ...
            for (ProcessVO importedProcessVO : importedPackageVO.getProcesses()) {
                ProcessVO reloaded = dataAccess.getProcessDefinition(
                        importedProcessVO.getProcessName(), importedProcessVO.getVersion());
        ...
        if (importedPackageVO.getRuleSets() != null) {
            ...
            for (RuleSetVO importedRuleSet : importedPackageVO.getRuleSets()) {
                RuleSetVO reloaded;
                if (dataAccess.getSupportedSchemaVersion() >= DataAccess.schemaVersion55) {
                    reloaded = dataAccess.getRuleSet(importedPackageVO.getId(),
                            importedRuleSet.getName());
                    if (reloaded == null) // need to verify whether the above is
                                          // even needed
                        reloaded = dataAccess.getRuleSet(importedRuleSet.getId());
                }
                else {
                    reloaded = dataAccess.getRuleSet(importedRuleSet.getName(),
                            importedRuleSet.getLanguage(), importedRuleSet.getVersion());
                }
        ...
        if (preexistingVersion > 0 && existingProcessVOs.isEmpty() && existingRuleSets.isEmpty()) {
            System.out.println("Saving Package...");
            if (isLocal())
                dataAccess.savePackageNoAudit(importedPackageVO); // force
                                                                  // associate
                                                                  // processes
            else
                dataAccess.savePackage(importedPackageVO); // force associate
                                                           // processes
        }
    }
